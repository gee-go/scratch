...
type BytesHashFn func(b []byte, seed uint64) uint64

var badByteHash = func(data []byte, seed uint64) uint64 {

	a, _ := Hash128x64(data, seed)
	return a
}
...
func TestSmhasherSanity(t *testing.T) {
	// testSmhasherSanity(t, BytesHash)
	testSmhasherSanity(t, func(data []byte, seed uint64) uint64 {
		for _, b := range data {
			seed ^= uint64(b)
		}
		return seed
...
func testSmhasherSanity(t *testing.T, bhash BytesHashFn) {
	...
	for k := 0; k < REP; k++ {
		for n := 0; n < KEYMAX; n++ {
			for i := 0; i < OFFMAX; i++ {
				...
				if bhash(b[PAD:PAD+n], 0) != bhash(c[PAD+i:PAD+i+n], 0) {
...
type HashSet struct {
	...
	strh func(b string, seed uint64) uint64
	bh   BytesHashFn
}
...
func newHashSet() *HashSet {
	return &HashSet{
		...
		n: 0,
		strh: func(data string, seed uint64) uint64 {
			return badByteHash([]byte(data), seed)
		},
		bh: badByteHash,
	}
}
...
func TestSmhasherAvalanche(t *testing.T) {
	...
	avalancheTest1(t, &BytesKey{make([]byte, 2), badByteHash})
	avalancheTest1(t, &BytesKey{make([]byte, 4), badByteHash})
	avalancheTest1(t, &BytesKey{make([]byte, 8), badByteHash})
	avalancheTest1(t, &BytesKey{make([]byte, 16), badByteHash})
	avalancheTest1(t, &BytesKey{make([]byte, 32), badByteHash})
	avalancheTest1(t, &BytesKey{make([]byte, 200), badByteHash})
...
func TestSmhasherWindowed(t *testing.T) {
	// windowed(t, &Int32Key{})
	// windowed(t, &Int64Key{})
	windowed(t, &BytesKey{make([]byte, 128), badByteHash})
}
